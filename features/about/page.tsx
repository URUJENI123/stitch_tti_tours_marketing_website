      ...
      <section className="bg-surface-container-low py-stack-lg">
        <div className="max-w-container-max mx-auto px-gutter">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {[
              {
                num: '01', icon: 'sentiment_very_satisfied', title: 'Customer Satisfaction',
              ...
              {
                num: '02', icon: 'public', title: 'Destination Promotion',
                desc: "Showcasing each destination's unique attractions and raising awareness of lesser-known places.",
              },
              {
                num: '03', icon: 'eco', title: 'Sustainable Tourism',
                desc: 'Minimizing negative impact on local environments and cultures while creating positive economic benefits.',
              },
              {
                num: '04', icon: 'groups', title: 'Employee Development',
                desc: 'Investing in staff training, career growth, and a positive work environment.',
              },
              {
                num: '05', icon: 'devices', title: 'Innovation & Technology',
                desc: 'Using technology to improve booking, communication, and tour management.',
              },
            ].map((goal, i) => (
              <div key={i} className="flex flex-col items-center text-center p-6 bg-white rounded-xl premium-shadow premium-shadow-hover transition-all">
                <div className="mb-4 text-teal opacity-80">
                  <span className="text-label-caps font-label-caps block mb-2">GOAL {goal.num}</span>
                  <span className="material-symbols-outlined text-4xl">{goal.icon}</span>
                </div>
                <h3 className="font-headline-md text-headline-md font-bold text-navy mb-2">{goal.title}</h3>
